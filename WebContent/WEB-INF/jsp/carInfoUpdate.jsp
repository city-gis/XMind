...
<script type="text/javascript">
	$(function() {
		...
		//--------------------------------------------
		//-----------------初始化车辆品牌、 系列和型号下拉列表 并默认选中-----
		/* [[品牌数据],[系列数组],[型号数组]]
		{品牌:[],系列:[],型号:[]} */
		$.post("updateVehicleInitBrand", {"typeId":"${vehicle.typeId}"}, function(data) {
			//根据json返回的数据初始化品牌下拉列表
			$.each(data.brandList, function(i, n) {
				$("#vehicleBrand").append(
						'<option value="'+n.id+'">' + n.name + '</option>');
			})
			//根据json返回的数据初始化车系下拉列表
			$.each(data.seriesList, function(i, n) {
				$("#vehicleStyle").append(
						'<option value="'+n.id+'">' + n.name + '</option>');
			})
			//根据json返回的数据初始化车型下拉列表
			$.each(data.typeList, function(i, n) {
				$("#vehicleType").append(
						'<option value="'+n.id+'">' + n.name + '</option>');
			})
			//设置品牌 车系 车型的下拉列表默认选中状态
			$("#vehicleBrand").val(data.brandId);
			$("#vehicleStyle").val(data.seriesId);
			$("#vehicleType").val("${vehicle.typeId}")
		}, "json");
		//---------------------------------------------------
		...
		<div class="main">
			...
			<div id="content">
				...
				<div class="rightContent center">
					<form use="inside" action="updateVechicle" method="post" >
						...
						<div class="table_con">
							<table width="100%" cellspacing="1" cellpadding="0" border="0"
								...
								<tbody>
									...
									<tr class="writebg">
										<td width="20%" align="left" class="boldtitle">会员ID:<span
											class="red">*</span></td>
										<td align="left" height="30">
											<div class="jianjv">
												<input name="url" type="text" value="P120400001" id="userId"
													maxlength="10" class="put_type">
											</div>
											<div class="jianjv">
												&nbsp;<input type="button" class="search_btn" value="检 索" />
											</div>
										</td>
									</tr>
									<tr>
								...
								<tbody>
									...
									<tr class="writebg">
										<td align="left" class="boldtitle" width="20%">VIN码：<span
											class="red">*</span></td>
										<td><input type="text" id="vin" value="${vehicle.vin }" maxlength="50"
									...
									<tr>
										...
										<td><input name="licensePlateNumber"
											class=" put_type_small" value="${vehicle.licensePlateNumber }" type="text" id="licensePlateNumber"
											maxlength="11" /></td>
									</tr>

									<tr>
										<td align="left" class="boldtitle">登记证号：</td>
										<td><input name="registerCode" value="${vehicle.registerCode }" class=" put_type_small"
									...
									<tr class="writebg">
										...
										<td><input name="mileage" class="numInput put_type_small"
											type="text" id="mileage" value="${vehicle.mileage }" maxlength="11" />&nbsp;公里</td>
									</tr>
									...
									<tr class="writebg">
										...
										<td><input name="registerDate" type="text"
											...
											class="put_type_small" value='<fmt:formatDate value="${vehicle.registerDate }"  pattern="yyyy-MM-dd"/>' readonly="readonly" /></td>
									</tr>
									...
									<tr>
										...
										<td><input type="text" class="numInput put_type_small"
											id="startingPrice" value="${vehicle.startingPrice }" maxlength="11"
									...
									<tr class="writebg">
										<td align="left" class="boldtitle">期望成交价格：<span
											class="red">*</span></td>
										<td><input type="text" class="numInput put_type_small"
											id="hopePrice" value="${vehicle.hopePrice}" maxlength="11" name="hopePrice" /> 元</td>
									</tr>
								</tbody>
							</table>
